...
var daap = {
    ...
    play: function(sessionId, callback) {
        var uri = urlModule.parse(daap.apiUrl() + 'ctrl-int/1/play?'
    ...
    stop: function(sessionId, callback) {
        var uri = daap.apiUrl() + 'ctrl-int/1/stop?session-id=' + sessionId;

        daap.requestProxyCallback(uri, callback);
    },

    /* Pause played current song in iTunes */
    volumeup: function(sessionId, callback) {
        var uri = daap.apiUrl() + 'ctrl-int/1/volumeup?session-id=' + sessionId;

        daap.requestProxyCallback(uri, callback);
    },

    /* Pause played current song in iTunes */
    volumedown: function(sessionId, callback) {
        var uri = daap.apiUrl() + 'ctrl-int/1/volumedown?session-id=' + sessionId;

        daap.requestProxyCallback(uri, callback);
    },

    /* Play next item in playlist */
    nextitem: function(sessionId, callback) {
        var uri = daap.apiUrl() + 'ctrl-int/1/nextitem?session-id=' + sessionId;

        daap.requestProxyCallback(uri, callback);
    },

    /* Play previous item in playlist */
    previtem: function(sessionId, callback) {
        var uri = daap.apiUrl() + 'ctrl-int/1/previtem?session-id=' + sessionId;
